...
double color[4][3] ={ {34., 94., 168.}, {65.,182.,196.}, {161., 218., 180.}, {255,237,160}};
...
void svQDOTDrawing::Texture_Size(GLfloat *m)
{
	...
	col[0] = 1; col[1] = 1; col[2] = 1; col[3] = 0.75;
	svVector4 col2;
	col2[0] = 1; col2[1] = 1; col2[2] = 1; col2[3] = 0.75;
...
void svQDOTDrawing::RenderMarkers(vector<int> index, GLfloat *m, int ENCODE_MODE)
{
  //cerr<<index[0]<<endl;
    GLdouble vmat[16], pmat[16], sx[3], sy[3], sz[3];
    GLint    viewport[4];
...
void svQDOTDrawing::RenderAnswerMarker(int index, GLfloat *m, int ENCODE_MODE)
{
  //cerr<<index[0]<<endl;
    GLdouble vmat[16], pmat[16], sx[3], sy[3], sz[3];
    GLint    viewport[4];
...
void svQDOTDrawing::Single_ColorSize(int exp, int coe)
{
	...
	glColor4f(color[exp][0]/255.,color[exp][1]/255.,color[exp][2]/255.,0.75);
	glBegin(GL_QUADS);
	glVertex2f(x,y);
	glVertex2f(x+2*size,y);
	glVertex2f(x+size*2,y-size*coe);
...
void svQDOTDrawing::Large_Axis_Legend()
{
    GLdouble vmat[16], pmat[16], sx[3], sy[3], sz[3];
    GLint    viewport[4];
	glEnable(GL_BLEND);
    ...
    float x, y;
    float width = 60 + 7 * 7 * 4;
    float height = 5 * 7 *6 + 7*7 + 11*7 + 8*7 + 9*7 + 10*7 + 30;
	...
	for(int i=0;i<strlen(str);i++)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, str[i]);	
	sprintf(str, "Digit");
	glRasterPos2f(x-20,y-height-10);
	for(int i=0;i<strlen(str);i++)
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, str[i]);	
	...
	for(int i=1;i<11;i++)
	{
		...
		sprintf(str, "9.99");
		if(i<=5)
			glRasterPos2f(x+5,y - 7*6*i);
		else
			glRasterPos2f(x+5,y - 7*6*5 - (6*7 + i*7)*(i-5)/2);
	...
	{
		sprintf(str, "%d", i);
		glRasterPos2f(x+30 + i * 9*7 + 3,y-5);
		for(int i=0;i<strlen(str);i++)
			glutBitmapCharacter(GLUT_BITMAP_HELVETICA_12, str[i]);	
	}
...
*/
	...
	glColor4f(color[exp][0]/255.,color[exp][1]/255.,
		color[exp][2]/255.,0.75);
...
void svQDOTDrawing::Color_Legend(int shift)
{
 ...
 for(int i=0;i<4;i++)
 {
   glColor4f(color[3-i][0]/255., color[3-i][1]/255., color[3-i][2]/255., 0.5);
   glPushMatrix();
   if(shift != 4)
...
void svQDOTDrawing::Single_TextureSize(int exp, int coe)
{
	...
	glBegin(GL_QUADS);
	glVertex2f(x,y);
	glVertex2f(x+2*size * exp/10.,y);
	glVertex2f(x+2*size * exp/10.,y-size*coe);
	glVertex2f(x,y-size*coe);
	glEnd();
