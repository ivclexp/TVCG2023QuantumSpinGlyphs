...
public:
	...
	void GetRotateFactor(float *r){for(int i=0;i<4;i++)r[i]=rotatefactor[i];}
	void GetZoomFactor(float &z){z=zoomfactor;}
	void GetPanFactor(float *p){p[0]=panfactor[0];p[1]=panfactor[1];}
	...
	void SetLogName(char *s){namelog=s;}
